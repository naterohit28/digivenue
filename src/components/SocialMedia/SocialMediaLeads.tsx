...
import { Instagram, Facebook, MessageCircle, MoreVertical, Reply, CheckCircle, LucideIcon, CheckCircle2 } from 'lucide-react';
...
export default function SocialMediaLeads({ leads: initialLeads }: SocialMediaLeadsProps) {
  const { showToast } = useToast();
  const [leads, setLeads] = useState(initialLeads);

  const handleReply = (id: string) => {
    setLeads(leads.map(l => l.id === id ? { ...l, status: 'replied' } : l));
    showToast('Reply sent successfully!');
  };

  const handleConvert = (id: string) => {
    setLeads(leads.map(l => l.id === id ? { ...l, status: 'converted' } : l));
    showToast('Lead converted to inquiry!');
  };
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button 
            ...
            className="px-4 py-2 bg-white border border-border rounded-xl text-xs font-bold hover:bg-bg transition-colors"
          >
            Filter Source
          </button>
          <button 
            onClick={() => showToast('Redirecting to account settings...', 'info')}
            className="px-4 py-2 bg-navy text-white rounded-xl text-xs font-bold shadow-lg shadow-navy/20 hover:bg-navy-mid transition-colors"
          >
            Connect Account
          </button>
        </div>
            ...
            <div key={lead.id} className="bg-white p-6 rounded-3xl border border-border card-shadow group hover:border-gold/30 transition-all">
              ...
              <div className="flex items-center gap-2">
                {lead.status === 'converted' ? (
                  <div className="flex-1 flex items-center justify-center gap-2 py-3 bg-jade/10 text-jade rounded-xl text-xs font-bold">
                  ...
                  <>
                    ...
                    <button 
                      ...
                      className="flex-1 flex items-center justify-center gap-2 py-3 bg-white border border-border text-navy rounded-xl text-xs font-bold hover:border-gold transition-all active:scale-95"
                    >
                      Convert
                    </button>
                  </>
      ...
      <div className="bg-gold-pale p-8 rounded-[40px] border border-gold/20 text-center">
        ...
        <button 
          onClick={() => showToast('Integration wizard coming soon!', 'info')}
          className="px-6 py-3 bg-navy text-white rounded-2xl text-sm font-bold shadow-xl shadow-navy/20 hover:scale-105 transition-transform"
        >
          Connect My Pages
        </button>
      </div>
